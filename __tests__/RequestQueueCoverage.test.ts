...
describe('RequestQueue Coverage Improvements', () => {
  ...
  it('should test binary insertion with many items in different order', async () => {
    ...
    for (let i = 0; i < 10; i++) {
      ...
      const promise = queue.enqueue(createConfig(priority, timestamp, `req${i}:${priority}:${timestamp}`)).then(() => {
  ...
  it('should respect queueDelay completely (delayed test)', async () => {
    ...
    // Enqueue a request
    await delayedQueue.enqueue(createConfig(AXIOS_RETRYER_REQUEST_PRIORITIES.MEDIUM, Date.now(), 'req1')).then(() => {
      endTime = Date.now();
    });
  ...
  it('should correctly handle critical request blocking case', async () => {
    ...
    await queue.enqueue(createConfig(AXIOS_RETRYER_REQUEST_PRIORITIES.HIGH, Date.now(), 'non-critical')).then(() => {
  ...
  it('should handle cancellation in the middle of a queue', async () => {
    ...
    for (let i = 0; i < 5; i++) {
      const promise = queue
        .enqueue(createConfig(AXIOS_RETRYER_REQUEST_PRIORITIES.MEDIUM, Date.now() + i, `req${i}`))
        .then(() => {
          results.push(`req${i}`);
          queue.markComplete();
        })
