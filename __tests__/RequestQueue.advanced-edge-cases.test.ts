...
describe('RequestQueue Advanced Edge Cases', () => {
  ...
  test('should handle queue overflow gracefully', async () => {
    ...
    for (let i = 0; i < 5; i++) {
      const promise = retryManager.axiosInstance.get(`/fast-request-${i}`).catch((error) => {
        // Requests beyond queue capacity (i > 1) are rejected immediately with QueueFullError.
        if (i > 1) {
  ...
  test('should handle request cancellation via AbortController', async () => {
    ...
    const cancelPromise = retryManager.axiosInstance
      .get('/to-be-cancelled', {
  ...
  test('should prioritize retries with higher priority', async () => {
    ...
    const firstPromise = retryManager.axiosInstance.get('/first-operation', {
      __axiosRetryer: { priority: AXIOS_RETRYER_REQUEST_PRIORITIES.CRITICAL },
    });
