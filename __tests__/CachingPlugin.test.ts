...
import { type CachingPluginOptions } from '../src/plugins/CachingPlugin/CachingPlugin';
...
describe('CachingPlugin', () => {
  ...
  test('should cache GET requests and serve subsequent calls from cache', async () => {
    ...
    expect(fakeLogger.debug.mock.calls.some((args) => args[0].includes('Cache hit'))).toBe(true);
  });
  ...
  test('should not serve cache if timeToRevalidate is exceeded', async () => {
    ...
    expect(fakeLogger.debug.mock.calls.some((args) => args[0].includes('Cache stale'))).toBe(true);
  });
  ...
  describe('Per-Request Cache Settings', () => {
    ...
    test('should store custom TTR when provided in __axiosRetryer.cachingOptions', async () => {
      ...
      expect(
        fakeLogger.debug.mock.calls.some((args) => args[0].includes('Caching response') && args[1]?.ttrMs === 5000),
      ).toBe(true);
    });
