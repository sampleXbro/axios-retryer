...
describe('Queue Delay Impact Tests', () => {
  ...
  test('should measure the impact of queue delay on retry performance', async () => {
    ...
    for (const delay of delayValues) {
      ...
      for (let i = 0; i < 5; i++) {
        directPromises.push(
          retryManager.axiosInstance.get(`/api/success-${i}`).then(() => {
      ...
      const avgDirectSuccess =
        Object.values(successTimes).reduce((sum, time) => sum + time, 0) / Object.values(successTimes).length;

      const avgRetrySuccess =
        Object.values(retrySuccessTimes).reduce((sum, time) => sum + time, 0) / Object.values(retrySuccessTimes).length;
    ...
    for (const delay of delayValues) {
      ...
      if (delay > 0) {
        const directOverhead =
          ((results[delay].firstSuccess - results[0].firstSuccess) / results[0].firstSuccess) * 100;
        const retryOverhead =
          ((results[delay].retriedSuccess - results[0].retriedSuccess) / results[0].retriedSuccess) * 100;
